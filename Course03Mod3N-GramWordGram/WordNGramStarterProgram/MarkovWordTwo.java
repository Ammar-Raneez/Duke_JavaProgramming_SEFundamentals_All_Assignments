...
/**
 * Same as for WordOne but adds two words 
 * @author (your name) 
...
public class MarkovWordTwo implements IMarkovModel{
    ...
    public int indexOf(String[] words, String target1, String target2, int start) {
        for(int i = start; i<words.length; i++) {
            if(i+1 > words.length-1) return -1;
    ...
    public String getRandomText(int numWords){
        ...
        for(int k=0; k < numWords-2; k++){
            ...
            sb.append(" ");
            key1 = key2;
            key2 = next;        //moving the two keys forwards       
        }
    ...
    private ArrayList<String> getFollows(String key1, String key2) {
        ...
        for(int i = 0; i < myText.length; i++) {
            ...
            if(index+key1.length() == myText.length || index == -1) {
                break;
            }
